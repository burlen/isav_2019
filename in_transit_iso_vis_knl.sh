...
if [[ "${O}" == "1" ]]
then
  S=./configs/iso_extract_opt_vis_knl.xml
else
...
cat ./configs/write_adios1_flexpath_iso_vis_knl.xml | sed "s/.*/$blu&$wht/"

srun -N ${RM} -n ${M} -r 0 oscillator -t ${T} -s ${L},${L},${L} -e 0,1,0,1,0,1 \
  -b ${M} -f ./configs/write_adios1_flexpath_iso_vis_knl.xml -g 1 -p 0  \
...
while [[ True ]]
do
  if [[ -e "data_iso_vis_knl.bp_writer_info.txt" ]]
  then
    break
  elif [[ ${delay} -le 0 ]]
  then
...
cat ${S} | sed "s/.*/$blu&$wht/"
cat ./configs/read_adios1_flexpath_iso_vis_knl.xml | sed "s/.*/$blu&$wht/"

srun -N ${RN} -n ${N} -r ${RM} SENSEIEndPoint -t ./configs/read_adios1_flexpath_iso_vis_knl.xml \
